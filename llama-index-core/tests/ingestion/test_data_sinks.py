...
def test_can_generate_schema_for_data_sink_component_type(
    ...
    schema = configurable_data_sink_type.value.schema(ref_template='#/$defs/{model}')  # type: ignore
    ...
    configured_schema = ConfiguredDataSink[component_type].schema(ref_template='#/$defs/{model}')  # type: ignore
